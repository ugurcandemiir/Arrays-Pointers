...
int main(int argc, char **argv) {
	...
	printFormat("vec3:",formatPtrVector(&vec3,4,fmtFloat));
	...
	printFormat("Pointer List:",formatPtrPlistScalar((void **)&plist2,4,fmtFloat));

	printFormat("Plist Vector:",formatPtrPlistVector((void **)&plistVector,3,4,fmtFloat));
	...
	for(row=0;row<3 && compare;row++){
		for(col=0;col<4 && compare;col++){
			if(prev!=NULL){
				 if((prev+1)!=*(plistVector)) compare=0;
			}
			prev = *(plistVector);
		}
	}
	...
	for(row=0;row<3 && compare ; row++){
		for(col=0 ; col<4 && compare ; col++){
			if(prevPtr!=NULL) if((prevPtr+1)!=(float *)&(plistPlist[row][col])) compare=0;
			prevPtr=(float *)&(plistPlist[row][col]);
		}
	}
